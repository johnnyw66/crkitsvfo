/*
 * "Sandwich Digital VFO" control software for CRKits KN-Q7A SSB transceiver kit 20 meter version and later on versions
 * 
...
unsigned long op_freq, band_low_limit = 14000000, band_high_limit, bfo_freq, bfo_low_limit, bfo_high_limit, temp_op_freq, temp_bfo_freq, target_freq = 10000000, rx_freq = 10000000;
...
void setup ()
{
  ...
  */
  ...
  if (digitalRead(JP))  //For 20 meter band, it is used for IF selection. High - IF 4096 kHz; Low - IF 4194.3 kHz
  {
    op_freq = 14300000;  //set op_freq default
    band_high_limit = 14350000;  //set band high limit to 14.35 MHz
    bfo_freq = 4096500;     //for IF = 4096 kHz, default to USB mode
    bfo_low_limit = 4092500;
    bfo_high_limit = 4098000;
  }
  else 
  {
    op_freq = 14200000;  //set op_freq to default
    band_high_limit = 14350000;  //set band high limit to 14.35 MHz
    bfo_freq = 4195000; //for IF = 4194.3 kHz, default to USB mode
    bfo_low_limit = 4191000;
    bfo_high_limit = 4196500;
  }
...
if (cal_steps < 128) {
 ...
 else mode = true; //otherwise stay in normal mode
}
